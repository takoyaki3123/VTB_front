...
import { eventDetailList, eventList } from '@/app/fakeData';
...
export default async function EventDetail({
  ...
  const eventData = eventDetailList[test.id];
  return (
    <div className="flex min-h-screen items-center justify-center font-sans">
      <div className="flex min-h-screen w-full max-w-5xl flex-col items-center justify-between sm:items-start">
        <div className="w-full">
          ...
          <div id="eventContainer" className="w-full flex flex-wrap">
            ...
            >
              <p className="w-full">
                {t('title')}：{eventData.name}&nbsp;&nbsp;&nbsp;
                {t('activityPeriod')}：{eventData.stime}~{eventData.etime}
              </p>
              <p>
                {t('offcialSite')}：
                <a href={eventData.link}>{eventData.link}</a>
              </p>
              <p className="w-full">{eventData.desc}</p>
            </div>
          </div>
          <div id="extractContent">
            <div id="eventContent" className="mt-4">
              <h1 className="text-lg ps-2 md:ps-0">{t('otherEvent')}</h1>
